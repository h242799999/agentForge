...
|----------|------|------|----------|----------|
| Critical | `PaymentViewModel.kt` | L42 | `!!` 强解包可能在网络失败时崩溃 | 改用 `?: return` 或 `requireNotNull` |
| Major | `OrderRepository.kt` | L87 | 协程在 catch 块中未取消，存在资源泄漏 | 在 finally 块中调用 `job.cancel()` |
...
| 检查项 | 状态 | 说明 |
|--------|------|------|
| 意图对齐 | ✅ 一致 | commit message 准确描述了变更内容 |
| 逻辑完整性 | ⚠️ 待确认 | 缺少对空购物车场景的处理 |
| 数据一致性 | ✅ 正常 | cache 和 DB 同步逻辑完整 |
| 回退安全性 | ✅ 可回滚 | 无数据迁移，回滚无风险 |
| 向后兼容 | ⚠️ 注意 | `checkout()` 新增了必填参数，下游需同步更新 |
...
| 文件 | 行号 | 问题描述 | 修复建议 |
|------|------|----------|----------|
| `PaymentUtils.kt` | L15 | 魔法数字 `3000`，含义不明 | 提取为常量 `PAYMENT_TIMEOUT_MS = 3000` |
| `CheckoutViewModel.kt` | L78 | 新增公共函数 `submitOrder()` 缺少 KDoc | 添加参数和返回值说明 |

> 无问题时填写：「diff 新增行符合代码规范」
...
检测到 Kotlin 文件变更（N 个 `.kt` 文件）。

建议后续运行深度 KMP/CMP 架构规范审查：
